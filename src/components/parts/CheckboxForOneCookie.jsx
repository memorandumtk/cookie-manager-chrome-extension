...
const CheckboxForOneCookie = ({ keyName, id, label, onChange, disabled }) => {
    return (
        <div className="w-full flex gap-4 justify-center items-center" key={keyName}>
            <input
                className="peer relative appearance-none shrink-0 w-4 h-4 border-2 border-blue-200 rounded-sm mt-1 bg-white z-10
                ...
                onChange={event => {
                    console.log(keyName + ' cookie is checked');
                    onChange(event.target.checked, keyName);
                }}
            />
            <svg
